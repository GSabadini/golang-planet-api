...
.PHONY: test
test: up ## Run golang tests
	${DOCKER_RUN} go test -cover -race ./...

.PHONY: test-report
test-report: up ## Run tests with HTML coverage report
	${DOCKER_RUN} go test -covermode=count -coverprofile coverage.out -p=1 ./... && \
	go tool cover -html=coverage.out -o coverage.html && \
	xdg-open ./coverage.html

.PHONY: test-report-func
test-report-func: up ## Run tests with func report -covermode=set
...
.PHONY: docker-bash
docker-bash: ## Exec docker sh for container project
	docker run --rm -it -e ENVIRONMENT=$(ENVIRONMENT) \
	-e SYSTEM=$(SYSTEM) \
	-e SYSTEM_VERSION=$(SYSTEM_VERSION) \
	-p 3000:3000 \
